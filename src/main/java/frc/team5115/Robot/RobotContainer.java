...
public class RobotContainer {
    ...
    public void configureButtonBindings() {
        new JoystickButton( joy, 1).onTrue(new IntakeForward(intakemotor));
        new JoystickButton( joy, 2).onTrue(new IntakeReverse(intakemotor));
        //new JoystickButton( joy, 1).whenHeld(new InstantCommand(intakemotor::forwardIntake)).whenReleased(intakemotor::stop);
        new JoystickButton( joy, 3).whileTrue(new InstantCommand(latch::open)).onFalse(new InstantCommand(latch::close));
